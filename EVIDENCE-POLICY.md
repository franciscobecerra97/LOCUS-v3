...
P2.1's descriptor disclosure analysis and unit positive control are design and
implementation checks, not collected C03/C06/C07/C21 evidence. P2.4 must use
the exact registered descriptor, pointer, manifest, and bundle bytes; include
the complete persistent role view; run the bounded networkless candidate test;
and retain only the privacy-safe observation defined by the security matrix.
