...
## Post-baseline integrated-system implementation note

The P7 same-process UI/client API remains a component profile and the frozen
Compose deployment remains a separate Yi-only historical profile. They are
not alternative final systems and are not reinterpreted by the new integrated
path.

D023 approves P7.5's new, separately versioned same-host reference system. Its
loopback UI/client gateway now uses authenticated remote adapters
