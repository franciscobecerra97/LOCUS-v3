...
| C06 | RecoveryDescriptor and recovery bundle do not add an offline predicate | Not part of baseline | P2.1 supplies strict schemas/codecs, forbidden-field analysis, canonical vectors, and a unit positive control; P2.4 still requires the exact disclosure snapshot, networkless candidate test, and evidence positive control |
| C07 | Clean client authenticates descriptor, bundle, and current epoch | Not part of baseline | P2.1 supplies externally rooted Ed25519 descriptor/pointer formats and exact cross-bindings; P2.2--P2.4 still require account-scope, substitution, stale, cross-user, cross-epoch, party-current-state, and trust-bootstrap tests |
...
| C13 | Storage adapters obey the same immutable backup/descriptor/bundle contracts | Supported for inherited filesystem and S3-compatible backup adapters only | P2.1 freezes the logical descriptor/pointer/manifest/bundle bytes; P2.3/P6 still require shared adapter conformance for local stores and the optional AWS S3 profile, without treating provider access control as authenticity |
...
| C21 | Recovery-bundle decoding and publication fail closed | Not part of baseline | P2.1 implements deterministic exact-member, path, duplicate, encryption/flags, compression/ratio/size, manifest/digest, signature, pointer, and positive-control unit tests; P2.3/P2.4 still require immutable publication/retry, stale-pointer, and scenario evidence |
