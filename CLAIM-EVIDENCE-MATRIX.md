...
| C07 | Clean client authenticates descriptor, bundle, and current epoch | Not part of baseline | P2.2 implements the exact installed operator/discovery/party trust configuration, optional receipt, short-lived party summaries, 4-of-5 current-state validation, endpoint/key/substitution/expiry/rotation negatives, and the P1.3 directory adapter over supplied bytes; P2.3/P2.4 still require admitted store retrieval plus stale/rollback scenario evidence |
...
| C21 | Recovery-bundle decoding and publication fail closed | Not part of baseline | P2.1 implements strict bundle decoding and P2.3 adds immutable filesystem/S3 publication, exact retry, stale-CAS, concurrency, ETag-race, corruption, oversize, and outage unit tests; P2.4 still requires aggregate scenario evidence |
