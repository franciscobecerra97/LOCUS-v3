...
| --- | --- | --- | --- |
| C01 | Existing v1 CuePolicy remains byte-compatible | Supported by the frozen cue-policy vectors | Reverify vectors and unchanged frozen-Yi behavior after interface refactor; separately verify that aPPSS consumes the same policy bytes only through its new suite-specific password domain |
...
| C23 | The aPPSS profile correctly recovers the exact high-entropy `S_R` only for the enrolled suite/password and a valid reconstruction subset | Not part of baseline; retained Yi results do not transfer | Approved D017 profile; native and cross-language vectors; all valid small subsets; authenticated distributed initialization; correct/wrong-input, malformed-state, cross-suite, cross-epoch, and end-to-end exact-key tests |
| C24 | Fewer than reconstruction threshold `k` aPPSS server states provide no local offline cue-verification predicate under the declared assumptions | Not part of baseline | Theorem 2/Figure 4 mapping plus exact-profile review; every evaluated below-`k` static coalition; matching cloud/descriptor state; networkless read-only candidate test; positive controls; explicit exclusion of side channels and online honest-server interactions |
| C25 | At least `k` compromised aPPSS server states enable offline dictionary testing but do not directly disclose `S_R` before a correct cue-derived password guess | Not part of baseline; the frozen Yi implementation has a different threshold-compromise failure mode | Reviewed threshold mapping `k=t_paper+1`; fixed aggregate-only exact-threshold/all-server aPPSS scenario; correct/wrong fixed candidates; Yi comparator that directly reconstructs its shared password scalar and recovery secret; no retained candidates or secrets; explicit conditional-entropy and unrate-limited-guessing limitation |
