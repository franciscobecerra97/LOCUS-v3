...
| C07 | Clean client authenticates descriptor, bundle, and current epoch | Not part of baseline | P2.2 implements trust/bootstrap, P2.3 exact storage/CAS, and P2.4 aggregate substitution/stale/rollback regression scenarios; admitted P3 retrieval and later evidence collection remain required |
...
| C09 | Enrollment uses authenticated confidential remote transport | Baseline uses trusted provisioner/direct volume writes | P3.1 implements the secret-free ordered client phase coordinator; P3.2 still requires cross-process provisioning and endpoint/recipient-binding tests |
...
| C13 | Storage adapters obey the same immutable backup/descriptor/bundle contracts | Supported for inherited filesystem and S3-compatible backup adapters only | P2.3 implements shared filesystem/S3-compatible descriptor, bundle, and current-pointer contracts with exact retry, no-list retrieval, ETag CAS, failure mapping, and unit conformance; P6 still requires optional AWS evidence, without treating provider access control as authenticity |
...
| C21 | Recovery-bundle decoding and publication fail closed | Not part of baseline | P2.1 strict decoding, P2.3 immutable publication/CAS, and P2.4 aggregate malformed/rollback regression scenarios pass; formal P9 evidence remains uncollected |
...
| C26 | Explicit Yi/aPPSS enrollment selection and same-suite/cross-suite successors cannot mix suites, fall back during recovery, or retire a predecessor before a recoverable successor exists | Not part of baseline | New suite/selector/epoch identifiers; descriptor-bound dispatch and no-fallback negatives; crash/retry matrix; Yi and aPPSS new-enrollment controls; Yi-to-Yi, aPPSS-to-aPPSS, Yi-to-aPPSS, and aPPSS-to-Yi fresh successor readiness/activation/retirement; frozen Yi regression |
